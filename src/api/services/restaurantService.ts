...
export const useRecommendedRestaurants = () => {
  return useQuery<Restaurant[], Error>("recommendedRestaurants", async () => {
    const response = await apiClient.get(`${RESTAURANTS_DOC_API}/sort/rate`);
...
export const useRestaurantSearch = (query: string) => {
  return useQuery<Restaurant[], Error>(
    ...
    async () => {
      const response = await apiClient.get(`${RESTAURANTS_DOC_API}/search`, {
        params: { query },
      });
      return response.data.data;
    },
...
export const useToggleFavorite = () => {
  ...
  return useMutation<void, Error, number>(
    async (id) => {
      const response = await apiClient.post(`${RESTAURANT_API}/${id}/favorite`);
      return response.data.data;
    },
    {
      onSuccess: () => {
...
export const useAddRestaurantMenu = () => {
  ...
  return useMutation<any, Error, { restaurantId: number; data: FormData }>(
    async ({ restaurantId, data }) => {
      const response = await apiClient.post(
        `${RESTAURANT_API}/${restaurantId}/menu`,
        data,
    ...
    {
      onSuccess: (_, { restaurantId }) => {
        queryClient.invalidateQueries(["restaurantMenus", restaurantId]);
      },
    }
  );
};
...
export const useUpdateMenu = () => {
  ...
  >(
    async ({ menuId, data }) => {
      const response = await apiClient.patch(`/api/v1/menu/${menuId}`, data, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
