...
const WheelOfFortune: React.FC = () => {
    ...
    const [selectedRestaurant, setSelectedRestaurant] =
        useState<RestaurantDetails | null>(null); // 세부사항 상태
    ...
    const fetchRestaurantDetails = (name: string) => {
        fetch(
            import.meta.env.VITE_CORE_API_BASE_URL +
                `/api/restaurants/detail?name=${encodeURIComponent(name)}`,
            {
                method: 'GET',
    ...
    const handleSpin = () => {
        if (!spinning && restaurants.length > 0) {
            ...
            (
                document.querySelector('.wheel') as HTMLElement
        ...
        <div className="WheelOfFortune">
                    ...
                    {restaurants.map((restaurant, index) => (
                        ...
                        >
                            ...
                            <span
                                ...
                                onClick={() =>
                                    fetchRestaurantDetails(restaurant.name)
                                }
                            >
                                {restaurant.name}
                            </span>
            ...
            {selectedRestaurant && (
                <div className="selected-restaurant">
                    <h2>세부정보</h2>
                    <h3>
                        <strong>이름:</strong> {selectedRestaurant.name}
                    </h3>
                    <p>
                        <strong>평점:</strong> ⭐ {selectedRestaurant.rating} /
                        5.0
                    </p>
                    <p>
                        <strong>리뷰:</strong> {selectedRestaurant.review}
                    </p>
                    <p>
                        <strong>카테고리:</strong> {selectedRestaurant.category}
                    </p>
                    <p>
                        <strong>키워드:</strong> {selectedRestaurant.keywords}
                    </p>
                    <p>
                        <strong>리뷰어:</strong> {selectedRestaurant.reviewer}
                    </p>
                    <p>
                        <strong>위치:</strong> 위도{' '}
                        {selectedRestaurant.latitude}, 경도{' '}
